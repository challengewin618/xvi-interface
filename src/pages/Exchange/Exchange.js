...
import {
  ...
  useChainId,
  useAccountOrders,
  getPageTitle, getTokenBalances, getServerUrl, BSC_TESTNET,
} from "../../lib/legacy";
...
export const Exchange = forwardRef((props, ref) => {
  ...
  const positionListUrl = getServerUrl(BSC_TESTNET, "/position_list?account=" + account + "&chainId=" + chainId);

  const fetcherUrl = (...args) => fetch(...args).then(res => res.json())

  const { data: positionData, error: positionDataError } = useSWR(positionListUrl, fetcherUrl);
  ...
  const LIST_SECTIONS = ["Positions"].filter(Boolean);
  ...
  const LIST_SECTIONS_LABELS = {
    ...
    Positions: (positionData && positionData.length) ? `Positions (${positionData.length})` : undefined,
  };
