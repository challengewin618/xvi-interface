...
const draw = () => {
  ...
  const { data: tokenAllowance } = useSWR(
    active && [active, chainId, tokenAllowanceAddress, "allowance", account, orderBookAddress],
    {
  ...
  useEffect(() => {
    const updateToValue = () => {
      if (tokenPrices) {
        const fromTokenPrice = tokenPrices[fromTokenAddress];
        const toTokenPrice = tokenPrices[toTokenAddress];

        if (hasLeverageOption && !isSwap) {
          setToValue(fromValue * leverageOption * fromTokenPrice / toTokenPrice);
        } else {
          setToValue(fromValue * fromTokenPrice / toTokenPrice);
        }
      }
    }
    ...
    const updateFromValue = () => {
      if (tokenPrices) {
        ...
        const toTokenPrice = tokenPrices[toTokenAddress];
        if (hasLeverageOption && !isSwap) {
          setFromValue(toValue * leverageOption * toTokenPrice / fromTokenPrice);
        } else {
          setFromValue(toValue * toTokenPrice / fromTokenPrice);
        }
      }
    }
...
*/
  ...
  const onSelectFromToken = async (token) => {
  ...
  const doSwap = async () => {
    ...
    if (fromTokenAddress !== AddressZero) {
      const erc20contract = new ethers.Contract(fromTokenAddress, WETH.abi, signer);
      if (tokenAllowance.lt(parseValue(fromValue, 18))) {
        setIsApproving(true);
        let tx = await erc20contract.approve(orderBookAddress, parseValue(fromValue, 18));
    ...
    if (fromTokenAddress === AddressZero) {
      await contract.swap(
        ...
        fromTokenAddress,
        toTokenAddress,
        parseValue(fromValue, 18),
        parseValue(toValue, 18),
        {
          value: parseValue(fromValue, 18)
        }
      );
    } else {
      await contract.swap(
  ...
  const doLeverage = async () => {
    ...
    leverage = bigNumberify(parseInt((hasLeverageOption ? leverageOption : 1) * BASIS_POINTS_DIVISOR));
  ...
  function setFromValueToMaximumAvailable() {
    ...
    const balance = formatAmountFree(fromBalance, fromToken.decimals, fromToken.decimals);
    setFromValue(balance);
    const fromTokenPrice = tokenPrices[fromTokenAddress === AddressZero ? nativeTokenAddress : fromTokenAddress];
    const toTokenPrice = tokenPrices[toTokenAddress === AddressZero ? nativeTokenAddress : toTokenAddress];
    if (hasLeverageOption) {
      setToValue(balance * leverageOption * fromTokenPrice / toTokenPrice);
    } else {
      setToValue(balance * fromTokenPrice / toTokenPrice);
    }
  }
